...
`prism-data-schema-forge` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies schema behavior through windowed input fixtures, with late-data behavior checks and fixture-scale datasets.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
